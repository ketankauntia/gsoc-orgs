 ...
 * 
 * Run with: npx ts-node scripts/generate-projects-data.ts
 */
...
interface ProjectYearPageData {
  ...
  charts: {
    top_technologies: Array<{ label: string; slug?: string; value: number; percentage?: number }>;
    orgs_with_most_projects: Array<{ label: string; slug?: string; value: number }>;
    project_difficulty_distribution?: {
      total: number;
      data: Array<{ label: string; value: number; percentage?: number }>;
...
function generateProjectsData() {
  ...
  for (const year of YEARS) {
    ...
    const yearlyData: YearlyData = JSON.parse(fs.readFileSync(yearlyFile, 'utf-8'));
    ...
    // Difficulty summary
    let difficultySummary: string | undefined;
    ...
    // Build output
    const output: ProjectYearPageData = {
      year: year,
      slug: `projects-${year}`,
